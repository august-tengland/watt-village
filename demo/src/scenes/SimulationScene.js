...
export default class SimulationScene extends Phaser.Scene {
    ...
    init() {
        ...
        this.solarPanelEffects = [10, 14]; //kWh (per hour)
    ...
    create () {
        ...
        // *****************************************************************
        this.runGuide = this.registry.get('runGuide');
        ...
        if(!this.runGuide) {
            ...
            this.guideDialog = this.getGuideDialog();
...
// *********************************************************************
    getGuideDialog() {
        ...
        console.log(json);
        const guideDialogData = json[this.currentDay];
    ...
    createGuide() {
        ...
        guide['dimensions'] = {x: 1920/2, y:1080/2, width: 700, height:400};
        ...
        guide['nextButton'] = this.add.image(0,0,"guideNextButton")
                                        .setInteractive()
                                        .on('pointerdown', () => this.updateGuide())
        ...
        const fadeInTween = this.getFadeTween([guide['container'],guide['text'],guide['nextButton'],guide['nextButtonLabel']],true,300);
    ...
    for (var apartment = 1; apartment <= 4; apartment++) {
        ...
        for (const [deviceName, DeviceValues] of Object.entries(apartmentDeviceInfo)) {
            if(DeviceValues['disable'] != null && DeviceValues['disable'].includes(apartment)) { //&& DeviceValues['disable'].contains(apartment)
                ...
                if(this.runGuide) DeviceValues['isIdleConsuming'] = false;
